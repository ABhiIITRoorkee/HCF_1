...
######
def create_adj_cat_large(node_features, n_components, device):
    ...
    adj_cat = np.zeros((n_nodes, n_nodes), dtype=float)

    for i in range(n_nodes):
        for j in range(i + 1, n_nodes):  # This ensures we only compute half and mirror it to save computation
            adj_cat[i, j] = adj_cat[j, i] = 1 if labels[i] == labels[j] else 0

    adj_cat = torch.FloatTensor(adj_cat).to(device)  # Ensure it's on the correct device
    print("Adjacency matrix shape:", adj_cat.shape)
    return adj_cat


# Determine if CUDA (GPU support) is available and set device accordingly
...
# print("shape of node features1",node_features.shape)
...
adj_cat = create_adj_cat_large(node_features, n_components=10, device=device)
